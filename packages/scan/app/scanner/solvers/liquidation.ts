...
export class LiquidationSolver {
  ...
  public async liquidate(blockNumber: number): Promise<Array<ShortfallRecord>> {
    ...
    logger.debug(
      `shorfallRecords: ${
        shorfallRecords.length ? JSON.stringify(shorfallRecords) : 'None'
      }`
    )
  ...
  async accountsLiquidity(): Promise<Array<ShortfallRecord>> {
    ...
    const asyncFilter = async (
      arr: Array<AccountId>,
      predicate: (a: AccountId) => Promise<ShortfallRecord>
    ) => {
    ...
    return await asyncFilter(borrowers, async (accountId) => {
      const accountLiquidity: [Liquidity, Shortfall] =
        await api.rpc.loans.getAccountLiquidity(accountId, null)
